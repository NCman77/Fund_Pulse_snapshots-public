...
function assessQuoteCandidate(candidate, { symbols = [], deadlineAt } = {}) {
...
function selectQuoteCandidate({ primary = null, backup = null, symbols = [], deadlineAt } = {}) {
  const primaryAssessment = primary ? assessQuoteCandidate(primary, { symbols, deadlineAt }) : null;
  const backupAssessment = backup ? assessQuoteCandidate(backup, { symbols, deadlineAt }) : null;
...
async function collectHedgedQuotes({
  ...
  if (!backupProvider) {
    ...
    deadline.cancel();
    controller.abort();
    return selectQuoteCandidate({ primary: primaryCandidate, symbols, deadlineAt });
  }
  ...
  if (first.type === 'primary' && !first.outcome.error) {
    const selected = selectQuoteCandidate({ primary: primaryCandidate, symbols, deadlineAt });
  ...
  const eligibleAfter = (task) => task.then(() => {
    const selected = selectQuoteCandidate({ primary: primaryCandidate, backup: backupCandidate, symbols, deadlineAt });
  ...
  const selected = await Promise.race([
    ...
    Promise.all([primaryTask, backupTask]).then(() => (
      selectQuoteCandidate({ primary: primaryCandidate, backup: backupCandidate, symbols, deadlineAt })
    )),
